...
- **Gretel `symptom_to_diagnosis`**  
  ...
  Primary natural-language symptom → diagnosis dataset.
...
- **MIMIC-IV-ED** — real triage labels, but credentialed access.  
  https://physionet.org/content/mimic-iv-ed/
- **MIETIC** — triage instruction corpus, but credentialed access.  
  https://physionet.org/content/mietic/
- **MIMIC-IV-Ext CDS** — rich triage/HPI set, but credentialed access.  
  https://physionet.org/content/mimic-iv-ext-cds/
- **DoReCo Gurindji / PARADISEC GK1** — useful only when audio access and permissions are confirmed.  
  https://doreco.huma-num.fr/languages/guri1247  
  https://catalog.paradisec.org.au/collections/GK1
- **CARPA manual** — strong remote NT context if access confirmed.  
  https://remotephcmanuals.com.au/manuals.html

### Reject for direct training

- **OpenSLR MUCS code-switching** — wrong languages; methodology reference only.  
  http://www.openslr.org/103
- **Google-UWA Aboriginal English dataset** — not released.  
...
2. Symptom2Disease
3. Healthcare Symptoms–Disease Classification Dataset
...
- symptom → diagnosis classifier prototype;
...
- `python_pipeline/02_finetune_whisper.py` stays manifest-first and blocked until waveform audio exists.
...
1. Download A1/A2/A3 and keep raw data under `python_pipeline/data/`.
